...
test('Returns error if pass invalid limit', async ({ page }) => {
  const [response] = await Promise.all([
    ...
    page.goto(
      'http://localhost:3000/?urls=https://github.com/alexcastrodev.png&limit=invalid'
    ),
  ])
...
test('Returns error if pass invalid columns', async ({ page }) => {
  const [response] = await Promise.all([
    page.waitForResponse((response) => response.status() === 422),
    page.goto(
      'http://localhost:3000/?urls=https://github.com/alexcastrodev.png&columns=invalid'
    ),
  ])
...
test('Returns error if pass invalid size', async ({ page }) => {
  const [response] = await Promise.all([
    page.waitForResponse((response) => response.status() === 422),
    page.goto(
      'http://localhost:3000/?urls=https://github.com/alexcastrodev.png&size=invalid'
    ),
  ])
